...
double lower_limit_roll = 0;   //The lowest possible output that the PID can produce
...
void loop() {
  ...
  if (error - lasterror < 20)
  {
    ...
    if (!mpuInterrupt && fifoCount < packetSize)
    {
        if (!gyro_stable)
        {
            if ((ypr2 > -0.05) && (ypr2 < 0.05))
            {
                myPID_roll.SetMode(MANUAL);
                indivSpeed(myservo1, 1330);
                indivSpeed(myservo3, 1330);
            }

            if ((ypr2 < -0.05) || (ypr2 > 0.05) && !error_correction)
            {
                myPID_roll.SetMode(AUTOMATIC);
            ...
            if ((ypr1 > -0.05) && (ypr1 < 0.05))
            {
                myPID_pitch.SetMode(MANUAL);
                indivSpeed(myservo2, 1330);
                indivSpeed(myservo4, 1330);
            }

            if (((ypr1 < -0.05) || (ypr1 > 0.05)) && !error_correction)
            {
                myPID_pitch.SetMode(AUTOMATIC);
