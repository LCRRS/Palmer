...
double Kp_roll = 5.0, Ki_roll = 0.0, Kd_roll = 2.0;     //Constant PID values determined experimentally through trial and error
double Kp_pitch = 5.0, Ki_pitch = 0.0, Kd_pitch = 2.0;

double lower_limit_roll = -100;   //The lowest possible output that the PID can produce
double upper_limit_roll = 100; //The maximum possible output that the PID can produce (anything higher will be set back to this value)
...
void warmup()
{
  ...
  myPID_pitch.SetSampleTime(10);
    ...
    {
        for (int speed = 1000; speed < 1550; speed++)
        {
            setSpeed(speed);
            delay(10);
        }
...
void loop() {
    ...
    ypr1 = (ypr[1] * 180/M_PI) - 0.85;
    ypr2 = (ypr[2] * 180/M_PI) - 2.66;
    ...
    if (!mpuInterrupt && fifoCount < packetSize)
    {
        if (millis() > 20000 && !gyro_stable)
        {
            ...
            pid_input_roll = ypr2;
            myPID_roll.Compute();
            speed_myservo1 = 1550 + pid_output_roll;
            speed_myservo3 = 1550 - pid_output_roll;
            indivSpeed(myservo1, speed_myservo1);
            indivSpeed(myservo3, speed_myservo3);
            Serial.print("ypr2  ");
            Serial.print(ypr2);
            Serial.print("      ");
            Serial.print(speed_myservo1);
            ...
            pid_input_pitch = ypr1;
            myPID_pitch.Compute();
            speed_myservo2 = 1550 + pid_output_pitch;
            speed_myservo4 = 1550 - pid_output_pitch;
