...
#define OFFSET_YPR1 0.4
#define OFFSET_YPR2 1.6
...
double pid_setPoint_pitch = 0;    //The desired value of the gyroscope. PID produces as an output until the value is reached
...
double Kp_roll = 3.0, Ki_roll = 0.0, Kd_roll = 0.0;     //Constant PID values determined experimentally through trial and error
double Kp_pitch = 3.0, Ki_pitch = 0.0, Kd_pitch = 0.0;

PID myPID_roll(&pid_input_roll, &pid_output_roll, &pid_setPoint_roll, Kp_roll, Ki_roll, Kd_roll, DIRECT);        //PID class object that is associated with respected variables for the roll plane
PID myPID_pitch(&pid_input_pitch, &pid_output_pitch, &pid_setPoint_pitch, Kp_pitch, Ki_pitch, Kd_pitch, DIRECT);
...
void warmup()
{
  myPID_roll.SetOutputLimits(LOWER_LIMIT, UPPER_LIMIT);
  myPID_pitch.SetOutputLimits(LOWER_LIMIT, UPPER_LIMIT);

  myPID_roll.SetSampleTime(SAMPLE_TIME); //Time to pass between the values of PID are recomputed (in milliseconds)
...
void loop() {
    ...
    ypr1 = (ypr[1] * 180/M_PI) - OFFSET_YPR1;
    ypr2 = (ypr[2] * 180/M_PI) + OFFSET_YPR2;
    ...
    if (!mpuInterrupt && fifoCount < packetSize)
    {
        if (!gyro_stable)
        {
            ...
            pid_input_roll = ypr2;
            myPID_roll.Compute();
            speed_myservo1 = 1330 + pid_output_roll;
            speed_myservo3 = 1330 - pid_output_roll;
            ...
            pid_input_pitch = ypr1;
            myPID_pitch.Compute();
            speed_myservo2 = 1330 + pid_output_pitch;
            speed_myservo4 = 1330 - pid_output_pitch;
